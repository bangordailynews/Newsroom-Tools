...
<wrapper>
	<table class="budget" style="width:100%">
		...
		<?php foreach( ( $terms = get_terms( 'desk', array( 'hide_empty' => false ) ) ) as $desk ) { ?>
			...
			<?php foreach( ( $authors = get_users( array( 'meta_key' => 'default_desk', 'meta_value' => $desk->term_id ) ) ) as $author ) { ?>
				...
				<?php if( ( empty( $stories ) || count( $stories ) == 0 ) && ( $desk->name == 'Other' || get_user_meta( $author->ID, 'is_ae', true ) ) ) continue; ?>
